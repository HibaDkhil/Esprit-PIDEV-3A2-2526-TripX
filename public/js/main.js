...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── Leaflet.js Map Integration for Destination Detail Page ──
    // Detects destination-detail page and dynamically loads Leaflet + renders map
    (function initDestinationMap() {
        // Only run on destination-detail page (has .detail-hero)
        const heroEl = document.querySelector('.detail-hero');
        if (!heroEl) return;

        // Extract coordinates from the Quick Info panel's "Coordinates" row
        const infoRows = document.querySelectorAll('[style*="justify-content:space-between"]');
        let lat = null, lng = null;
        infoRows.forEach(row => {
            const label = row.querySelector('span');
            if (label && label.textContent.trim() === 'Coordinates') {
                const coordSpan = row.querySelectorAll('span')[1];
                if (coordSpan) {
                    const parts = coordSpan.textContent.split(',').map(s => parseFloat(s.trim()));
        ...
        // Extract destination name from the hero title
        const titleEl = document.querySelector('.detail-title');
        const destName = titleEl ? titleEl.textContent.trim() : 'Destination';
        ...
        leafletJS.onload = function () {
            // Create the map section container
            const mapSection = document.createElement('section');
            mapSection.className = 'section';
            mapSection.id = 'destination-map-section';
            mapSection.innerHTML = `
                <div class="container">
                    <div class="section-label reveal"><span>Location</span></div>
                    <h2 class="display-md reveal" style="margin-bottom:24px">
                        Explore <span class="gradient-text">${destName}</span> on the Map
                    </h2>
                    <div id="destination-leaflet-map" style="
                        width: 100%;
                        height: 450px;
            ...
            // Insert the map section before the Activities section (second <section>)
            const sections = document.querySelectorAll('section.section');
            if (sections.length >= 2) {
                sections[1].parentNode.insertBefore(mapSection, sections[1]);
            } else {
            ...
            const map = L.map('destination-leaflet-map', {
            ...
            const markerIcon = L.divIcon({
                ...
                html: `<div style="
                    ...
                    box-shadow: 0 4px 12px rgba(14,165,255,0.4);
            ...
            L.marker([lat, lng], { icon: markerIcon })
                .addTo(map)
                .bindPopup(`
                    <div style="font-family: 'Montserrat', sans-serif; text-align: center; padding: 4px 0;">
                        <strong style="font-size: 14px;">${destName}</strong><br>
            ...
            // Fix map rendering when it becomes visible (scroll reveal)
            ...
            mapSection.querySelectorAll('.reveal').forEach(el => mapObserver.observe(el));
    ...
    // ── Leaflet.js Map Integration for Activities Page ──
    // Moved to activities.html.twig for better performance and to avoid conflicts
