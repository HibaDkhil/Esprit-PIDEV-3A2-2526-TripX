...
class LiveController extends AbstractController
{
    ...
    public function list(
        ...
        LiveReactionRepository $reactionRepository
    ): JsonResponse {
        /** @var User|null $me */
        $me = $this->getUser();
    ...
    public function react(
        ...
        $reaction->setLiveSession($session);
        $reaction->setUser($me);
    ...
    public function token(int $id, LiveSessionRepository $sessionRepository): JsonResponse
    {
        ...
        $wsUrl = trim((string) ($_ENV['LIVEKIT_URL'] ?? $_SERVER['LIVEKIT_URL'] ?? ''));
        ...
        $token = JWT::encode($claims, $apiSecret, 'HS256', $apiKey);
    ...
    private function removeSessionWithRelations(LiveSession $session, EntityManagerInterface $em): void
    {
        ...
        $em->createQuery('DELETE FROM App\\Entity\\LiveComment c WHERE c.liveSession = :session')
            ...
            ->execute();
        $em->createQuery('DELETE FROM App\\Entity\\LiveReaction r WHERE r.liveSession = :session')
            ->setParameter('session', $session)
            ->execute();
